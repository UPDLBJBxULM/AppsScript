function GenPDFMRK04() {
  ...
  // Mengakses sheet MRK04 untuk mengambil data E5
  var generateSheet = ss.getSheetByName('MRK-04'); // Ganti dengan nama sheet yang sesuai
  
  // Cek apakah sheet ditemukan
  if (!generateSheet) {
    Logger.log('Sheet MRK-04 tidak ditemukan.');
    SpreadsheetApp.getUi().alert('Sheet MRK-04 tidak ditemukan.');
    return; // Hentikan eksekusi script jika sheet tidak ada
  }

  // Mendapatkan data periode dari E5
  var periodeData = generateSheet.getRange('E5').getValue();
  
  // Log data untuk melihat nilai sebelum perubahan
  Logger.log('Data E5: ' + periodeData);
  
  // Ekstrak tahun dan semester dari periodeData
  var year = periodeData.match(/Semester (\d+) (\d{4})/);
  if (year) {
    var semester = year[1].padStart(2, '0'); // Menambahkan nol di depan jika semester < 10
    year = year[2]; // Ambil tahun
  } else {
    Logger.log('Format data E5 tidak sesuai.');
    SpreadsheetApp.getUi().alert('Format data E5 tidak sesuai.');
    return; // Hentikan eksekusi jika format tidak sesuai
  }
  
  // Format nama PDF sesuai dengan data yang ditemukan
  var pdfName = 'MRK-04_' + year + '_' + semester + '.pdf';
  ...
  var folderId= '1Ev06HI5BV09fym_FoqzWmgnTdznKOrgP';
