function onFormSubmit_checkOut(e) {
  if (!e) {
  ...
  try {
    ...
    var guestCOSheet = ss.getSheetByName("GUESTCO");
    var roomPlotSheet = ss.getSheetByName("ROOMPLOT");

    if (!guestCOSheet || !roomPlotSheet) {
      Logger.log("Sheet 'GUESTCO' atau 'ROOMPLOT' tidak ditemukan.");
      return;
    }
    ...
    var publikasiSheet = ss.getSheetByName("Publikasi_CheckOut");
    ...
    try {
      ...
      // Cari baris di sheet GUESTCO berdasarkan guestID
      var guestCOData = guestCOSheet.getRange(2, 1, guestCOSheet.getLastRow() - 1, 3).getValues(); // Kolom A-C
      var guestCORowIndex = -1;

      for (var i = 0; i < guestCOData.length; i++) {
        var row = guestCOData[i];
        var guestIDInGuestCO = row[1]; // Asumsikan guestID ada di kolom B (indeks 1)

        if (guestIDInGuestCO === formGuestID) {
          guestCORowIndex = i + 2; // Offset header dan index dimulai dari 0
          break;
        }
      }

      if (guestCORowIndex === -1) {
        Logger.log("GuestID tidak ditemukan di sheet GUESTCO.");
        return;
      }

      // Tandai status di sheet GUESTCO sebagai SENT
      guestCOSheet.getRange(guestCORowIndex, 3).setValue("SENT"); // Asumsikan kolom status ada di kolom C
      Logger.log("Status di sheet GUESTCO diperbarui menjadi SENT untuk GuestID: " + formGuestID);
    } catch (error) {
